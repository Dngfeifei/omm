...
import { addInsuranceAgent } from '/api/insurance'

class InsuranceAgentItem extends Component{
    ...
    state = {
        rules: [{
            ...
            render: _ => <Select size='small' mode="multiple" style={{width: 200}}
                                 showSearch
            ...
            label: '有效期(起)',
            key: 'speriod',
            option: { rules: [{required: true, message: '请选择有效起始月!'}] },
            render: _ => <MonthPicker placeholder="选择月份" />
        },{
            label: '有效期(止)',
            key: 'operiod',
            option: { rules: [{required: true, message: '请选择有效截止月!'}] },
            render: _ => <MonthPicker placeholder="选择月份" />
        },{
            label: '备注',
            key: 'remark',
        ...
        if (!this.state.lock) {
            ...
            this.props.form.validateFieldsAndScroll(null, {}, (err, val) => {
                if (!err || !Object.getOwnPropertyNames(err).length) {
                    ...
                    if(params.city) params.city = params.city.join(',')
                    if (this.props.config.type === 'edit') {
                        params.id = this.props.config.item.id
                    }
                    addInsuranceAgent(params)
                        .then(res => {
...
const InsuranceAgentForm = Form.create()(InsuranceAgentItem)
export default InsuranceAgentForm