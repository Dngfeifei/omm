...
import PreviewPublicWall from '/components/previewPublicWall.jsx'
import { handleTreeData, assignKey } from '/api/tools'


class PropagandaView extends Common{
	...
	state = {
		...
		columns: [{
			...
			title: '操作',
			dataIndex: 'operator',
			render: (t, r) => r.children && r.children.length>0 ? null : <div><a style={{display: 'inline-block'}} onClick={async _ => {this.openPicModel(r)}}>查看下载</a></div>
		}],
		selectedtable: false,
		loading: true,
		tabledata: [],
  ...
  // 打开资料上传
	...
	rendermodal = _ => <div>
	<PreviewPublicWall onCancel={_ => this.cancelform('picmodelConf')}
		id={this.state.id}
