...
import { getcertpaperlist, returnPaper, renewPaper, renewMutiPaper, returnMutiPaper } from '/api/staff'
...
class Contpaper extends Common{
	...
	async componentWillMount () {
		this.search()
		getParam({code: 'staff_renew_days'}).then(res => {
			this.setState({staff_renew_days: Number(res.data) })
		})
	}
	...
	state = Object.assign({}, this.state, {
		...
		columns: [{
			title: '原件名称',
			dataIndex: 'name',
			render: t => <div style={{width: 260, wordWrap: 'break-word'}} >{t}</div>
		},{
			title: '人员',
			dataIndex: 'staffname'
		},{
	...
	getOperate = r => {
		if (this.state.isAdmin){
			...
			if ((r.status == 1 || r.status == 3) && r.papernum > r.backnum)
				return r.renew == 0 ? <a style={{display: 'inline-block', width: 50}} onClick={_ => this.openmodal(r)}>续借</a> : null
		}
	}
	...
	//续借
	renewPaper = r => {
		renewPaper({detailId: r.id, days: this.state.staff_renew_days}).then(res => {
			if(res.code == 200){
				message.success('操作成功')
				this.search()
			}
	...
	renderSearch = _ => <div className="mgrSearchBar">
		<Input 
			...
			style={{width: 300}}
			addonBefore="关键字" placeholder="证书号/证书名称/流程编号/项目编号" />
