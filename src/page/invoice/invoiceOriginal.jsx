...
import { Button, Modal, Icon, Select, Input, DatePicker, message, Popconfirm, InputNumber, Tooltip, Table } from 'antd'
const { TextArea } = Input
const ButtonGroup = Button.Group
import { getBorrowPaperList, returnPaper, returnMutiPaper, invoicelist } from '/api/invoice'
import Common from '/page/common.jsx'
import moment from 'moment'
import { StatusMap } from '/api/tools'
import { getParam } from '/api/global'

class Certpaper extends Common{
	...
	state = Object.assign({}, this.state, {
		...
		columns: [{
			...
			title: '合同名称',
			dataIndex: 'vcontname',
			render: t => <div style={{width: '260px'}}>{t}</div>
		},{
			title: '项目编号',
			dataIndex: 'projcode'
		},{
			...
			dataIndex: 'operate0',
			align: 'center',	
			render: (t, r) => <a style={{display: 'inline-block', width: 60}} onClick={_ => this.openInvoice(r)}>查看发票</a>
		},{
			title: ' 操作 ',
			dataIndex: 'operate',
			align: 'center',	
	...
	//打开发票页面
	openInvoice = r => {
		this.setState({invdatas: [], invisible: true})
		invoicelist({type: 1, projcode: r.projcode, offset: 0, limit: 9999}).then(res => {
			this.setState({invdatas: res.data.records})
		})
	}
	//获得状态
	getStatusName = r => {
	...
	rendermodal = _ => <div>
		...
		<Modal title='发票列表'
		  ...
		  mask={true}
		  width={800}
		  onCancel={_ => this.setState({invisible : false})}
		  footer={null}
		>
		<Table columns={this.state.invcolumns} dataSource={this.state.invdatas} locale={{emptyText: '暂无数据'}} pagination={false} />
		</Modal>
		<Modal title="归还"
