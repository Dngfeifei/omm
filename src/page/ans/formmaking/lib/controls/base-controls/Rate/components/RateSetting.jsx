...
const RateSetting = ({ control, updateFormModel }) => {
  ...
  return (
    <div>
      ...
      <FormAttrItem label="隐藏标签">
        ...
        <FormAttrItem label="允许半选">
            <Switch
                checked={options.allowHalf}
                onChange={(checked) => updateOptions({ allowHalf: checked })}
            />
        </FormAttrItem>
        <FormAttrItem label="默认值">
            <StarRate max={options.max} value={options.defaultValue || 0}
                      ...
                      <div onClick={() => updateOptions({defaultValue: 0})}>清空</div>
        </FormAttrItem>
