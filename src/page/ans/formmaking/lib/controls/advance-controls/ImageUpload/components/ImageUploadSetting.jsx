...
import UploadMetaSetting from '@/page/ans/formmaking/lib/controls/common/UploadMetaSetting.jsx';
...
const ImageUploadSetting = ({ control, updateFormModel }) => {
  ...
  return (
    <div>
      ...
      <FormAttrItem label="大小">
        宽度
        <Input
          value={options.size.width}
          onChange={(e) => {
            updateOptions({ size: { ...options.size, width: e.target.value }});
          }}
        />
        高度
        <Input
          ...
          onChange={(e) => {
            updateOptions({ size: { ...options.size, height: e.target.value } });
          }}
        />
      </FormAttrItem>
      ...
      <FormAttrItem label="最小值">
        <InputNumberPlus
          step={1}
          value={options.min}
          onChange={(value) => updateOptions({ min: value })}
        />
      </FormAttrItem>
      ...
      <FormAttrItem label="最大上传数">
        <InputNumberPlus
          step={1}
          value={options.limit}
          onChange={(value) => updateOptions({ limit: value })}
        />
      </FormAttrItem>

      <UploadMetaSetting options={options} updateOptions={updateOptions} />
      ...
      <FormAttrItem label="操作属性">
        <WrapFlex>
          ...
          <div>
            <Checkbox
              checked={options.isDelete}
              onChange={(e) => updateOptions({ isDelete: e.target.checked })}
            >
              可删除
            </Checkbox>
          </div>
          <div>
            <Checkbox
              checked={options.isEdit}
              onChange={(e) => updateOptions({ isEdit: e.target.checked })}
            >
              可编辑
            </Checkbox>
          </div>
