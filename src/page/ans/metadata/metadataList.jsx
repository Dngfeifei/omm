...
import TableList from "././EditableCell/tableList.jsx";
// 分页组件
import Pagination from "@/components/pagination/index";
// 引入 API接口
import { getCategoryTree, addTree, updateTree, getInfoTree, getLists, deleteTree } from '/api/metaData'
...
class MetaData extends Component {
    state = {
        ...
        addrules: [{
            label: '元数据类别名称',
            key: 'metaCategoryName',
            option: {
                rules: [{
                    required: true, message: '元数据类别名称不能为空',
                }]
            },
    ...
    assignment = (data) => {
        data.forEach((obj, i) => {
            obj.key = obj.id;
            if (obj.hasOwnProperty("metaCategoryName")) {
                obj.title = obj.metaCategoryName;
            }
            if (obj.hasOwnProperty("children")) {
                if (obj.children.length > 0) {
                    this.assignment(obj.children)
                }
            } else {
                return
            }
    ...
    init = () => {
        // 获取元数据类型树
        getCategoryTree({ metaCategoryName: this.state.searchValue }).then(res => {
            if (res.code === 200) {
    ...
    // 获取参数类别详情
    getInfo = id => {
        getInfoTree({ id }).then(res => {
            if (res.code === 200) {
                if (this.state.middleStatus === 'addTree') {
                    this.props.form.setFieldsValue({
                        metaCategoryName: '',
                        parentName: res.data.metaCategoryName
                    })
                } else if (this.state.middleStatus === 'editTree') {
                    this.props.form.setFieldsValue({
                        metaCategoryName: res.data.metaCategoryName,
    ...
    onSelect = (selectedKeys, info) => {
        ...
        let data = Object.assign({}, this.state.form, { metaCategoryId: id })
    ...
    getFormData = _ => {
        ...
        this.props.form.validateFieldsAndScroll(null, {}, (err, val) => {
            if (!err || !Object.getOwnPropertyNames(err).length) {//校验完成执行的逻获取合并后的表单数据
    ...
    handleOk = () => {
        ...
        let paramsName = this.getFormData().metaCategoryName;
        if (this.state.middleStatus === 'addTree') {
            let param = {
                parentId: id,
                metaCategoryName: paramsName
            }
    ...
    onPageChange = (page, pageSize) => {
        let data = Object.assign({}, this.state.form, { offset: (page - 1) * pageSize })
    ...
    onShowSizeChange = (current, pageSize) => {
        let data = Object.assign({}, this.state.form, { offset: 0, limit: pageSize })
                ...
                <Row gutter={24} className="main_height">
                    ...
                    <Col span={19} className="gutter-row main_height" style={{ padding: '0 10px 0', backgroundColor: 'white', display: 'flex', flexDirection: 'column', flexWrap: 'nowrap' }}>
                        <div className="tableParson" style={{ flex: 'auto' }} ref={(el) => this.tableDom = el}>
                            <TableList scroll={h} style={{ marginTop: '16px', padding: '0px 15px', height: h, overflowY: 'auto' }} parentValueID={this.state.form.metaCategoryId} data={this.state.tabledata} handleChange={this.handleChange}>
...
const MetaDataForm = Form.create()(MetaData)
