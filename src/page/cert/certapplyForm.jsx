...
import Certpaperselect from '/page/cert/certpaperselect.jsx'
import Certselect from '/page/cert/certselect.jsx'
import { uploadAttr, getParam } from '/api/global'
import moment from 'moment'

class CertapplyItem extends Component{
	...
	setChecked = val => {
		...
		if(val == '原件'){
			this.setState({type: 'certp'})
			this.state.rules.forEach(item => {
				if(item.key == 'borrowDate' || item.key == 'returnDate' || item.key == 'getType'){
					item.hidden = false
				}
			})
		}else{
			this.setState({type: 'certe'})
			this.state.rules.forEach(item => {
				if(item.key == 'borrowDate' || item.key == 'returnDate' || item.key == 'getType'){
					item.hidden = true
				}
	...
	addmodel = _ => {
		if(this.state.type == 'certp'){
			this.setState({visible: true})
		}
		if(this.state.type == 'certe'){
	...
	rendermodal = _ => 
		...
		<Modal title="选择资料"
		  ...
		  width={1200}
		>
			<Certpaperselect selectedkey={this.state.selectedkeys} selectdone={this.selectdone} onCancel={_ => this.setState({visible: false})}  />
		</Modal> 
		<Modal title="选择资料"
			footer={null}
		  ...
		  width={1200}
		>
			<Certselect selectedkey={this.state.selectedkeys} selectdone={this.selectdone} onCancel={_ => this.setState({evisible: false})}  />
		</Modal> 
		</div>
	...
	render = _ => {
      ...
      renderItem={item => (<List.Item actions={[<a title="删除"><Icon type="delete" onClick={_ => this.removedone(item.key)} /></a>]}>{item.name}-{this.state.type=='certp' ? item.paperName : item.code}</List.Item>)}
    	/>
...
const CertapplyForm = Form.create()(CertapplyItem)
export default CertapplyForm