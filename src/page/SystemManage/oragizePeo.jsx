...
//工程师选择器
import PostArea from '@/components/selector/engineerSelector2.jsx'
...
class systempeo extends Component {
    ...
    postSave = (resultID, result) => {
        ...
        const { selectedTreeId } = this.state;
        saveEditJG({ id: selectedTreeId, leaders: resultID }).then(res => {
            if (res.success == 1) {
                this.getLeader();
                this.postClose();
    ...
    //打开工程师选择器
    plus = () => {
        let { selectedTreeId } = this.state;
        if (!selectedTreeId) {
            message.destroy()
            message.warning("请选择机构后再设置部门领导！")
            return
        }
            ...
            <Row className="my_height" style={{ paddingTop: '0px' }}>
                ...
                <Col span={19} className="my_height" style={{ paddingLeft: '10px', paddingTop: '8px', background: "#fff" }}>
                    ...
                    <Row type="flex" align="middle" style={{ height: 50, border: '0px solid red', marginTop: -50, width: 841 }}>
                        <span style={{ fontWeight: 100, color: 'rgba(0, 0, 0, 1)', marginRight: "10px" }}>
                            部门领导：{this.state.leader.length ? this.state.leader.join(",") : '---'}
                        </span>
                        <span>
