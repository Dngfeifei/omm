...
import { Input, Form, Row, Button, message, Card, Table, Select, Icon } from 'antd'
const { Option } = Select;
...
class People extends Component {
	...
	// 选中数据追加
	addResult = () => {
		let { resultID, finalResultID, result, finalResult } = this.state
		if (finalResultID.indexOf(resultID[0]) > -1) {
			return
		}
		finalResultID.push(resultID[0])
	...
	// 已选中数据删除
	delResult = () => {
		let { finalResultID, finalResult, choiceKey } = this.state
		if (choiceKey < 0) {
			return
		}
		finalResultID.splice(choiceKey, 1)
		finalResult.splice(choiceKey, 1)
		this.setState({
			finalResultID,
			finalResult,
			choiceKey: -1
	...
	// 提交
	onSubmit = () => {
		let { finalResultID, finalResult } = this.state;
		if (finalResultID.length > 0) {
			this.props.onOk(finalResultID, finalResult)
			this.props.onCancel()
		} else {
			message.destroy()
			message.error("未选择数据")
		...
		return <div>
			...
			>
				<div style={{ display: 'flex', height: "400px" }}>
					<Card bodyStyle={{ height: "100%" }} style={{ flex: 2, marginRight: "20px", overflow: "auto" }}>
						<TreeParant
							edit={false}
					...
					<div style={{ height: "100%", flex: 0.5, overflow: "auto", position: "relative" }}>
						<Icon onClick={this.addResult} style={{ fontSize: "28px", cursor: "pointer", position: "absolute", left: "50%", top: "40%", transform: "translate(-50%, -50%)" }} type="right-circle" />
						<Icon onClick={this.delResult} style={{ fontSize: "28px", cursor: "pointer", position: "absolute", left: "50%", top: "53%", transform: "translate(-50%, -50%)" }} type="left-circle" />
					</div>
					<Card bodyStyle={{ height: "100%", padding: "10px" }} style={{ flex: 1, marginRight: "20px", overflow: "auto" }}>
						{
							this.state.finalResult.map((item, i) => {
								return <p style={this.state.choiceKey == i ? { backgroundColor: "##bae7ff", marginBottom: "5px", cursor: "pointer" } : { marginBottom: "5px", cursor: "pointer" }} key={i} onClick={_ => this.choice(i)}>{item.realName}</p>
							})
						}
					</Card>
				</div>
